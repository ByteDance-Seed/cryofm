# CryoFM1 (S)Small input shape 64, 1.5Apix, model architecture p4 d2
patch_size = 64

train_pipeline = [
    dict(type="LoadMapFromFile"),
    dict(type="RandomRotation3D", keys=["map_data"], p=1.0),
    dict(type="ClipNorm3D", min_value=0, max_percentile=99.999),
    dict(type="Pad3D", keys=["map_data"], size=patch_size * 2),
    dict(type="CreateProbabilityMap3D", threshold=0.1, dilation_iter=3),
    dict(type="WeightedPatches3D", patch_size=patch_size, probability_map_name="prob_map", num_patches=2),
...
train_dataset = dict(
        type='EMDBMapDataset',
        data_root='path/to/cryofm1_1-5apix_dataset/',
