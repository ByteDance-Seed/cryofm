...
def central_slice_sample_ndhwc_gen_fwd_kernel(
    ...
    out_base = base_pose + oh + ow + chan64 * output_stride_c
    ...
    base000c = base000 + chan64 * input_stride_c
    base001c = base001 + chan64 * input_stride_c
    base010c = base010 + chan64 * input_stride_c
    base011c = base011 + chan64 * input_stride_c
    base100c = base100 + chan64 * input_stride_c
    base101c = base101 + chan64 * input_stride_c
    base110c = base110 + chan64 * input_stride_c
    base111c = base111 + chan64 * input_stride_c

    v000 = tl.load(input_ptr + base000c, mask=m000, other=0.0).to(tl.float32)
    v001 = tl.load(input_ptr + base001c, mask=m001, other=0.0).to(tl.float32)
    v010 = tl.load(input_ptr + base010c, mask=m010, other=0.0).to(tl.float32)
    v011 = tl.load(input_ptr + base011c, mask=m011, other=0.0).to(tl.float32)
    v100 = tl.load(input_ptr + base100c, mask=m100, other=0.0).to(tl.float32)
    v101 = tl.load(input_ptr + base101c, mask=m101, other=0.0).to(tl.float32)
    v110 = tl.load(input_ptr + base110c, mask=m110, other=0.0).to(tl.float32)
    v111 = tl.load(input_ptr + base111c, mask=m111, other=0.0).to(tl.float32)
...
def central_slice_sample_ndhwc_gen_bwd_input_kernel(
    ...
    chan64 = chan.to(tl.int64)
    grad = tl.load(
        grad_output_ptr + base_pose + oh + ow + chan64 * grad_output_stride_c,
        mask=mask,
        other=0.0,
    ).to(tl.float32)
    ...
    tl.atomic_add(grad_input_ptr + base000 + chan64 * grad_input_stride_c, w000 * grad, mask=m000)
    tl.atomic_add(grad_input_ptr + base001 + chan64 * grad_input_stride_c, w001 * grad, mask=m001)
    tl.atomic_add(grad_input_ptr + base010 + chan64 * grad_input_stride_c, w010 * grad, mask=m010)
    tl.atomic_add(grad_input_ptr + base011 + chan64 * grad_input_stride_c, w011 * grad, mask=m011)
    tl.atomic_add(grad_input_ptr + base100 + chan64 * grad_input_stride_c, w100 * grad, mask=m100)
    tl.atomic_add(grad_input_ptr + base101 + chan64 * grad_input_stride_c, w101 * grad, mask=m101)
    tl.atomic_add(grad_input_ptr + base110 + chan64 * grad_input_stride_c, w110 * grad, mask=m110)
    tl.atomic_add(grad_input_ptr + base111 + chan64 * grad_input_stride_c, w111 * grad, mask=m111)