...
__all__ = ["central_slice_embed_batched_kernel"]
...
def central_slice_embed_batched_kernel(
    ...
    if HAS_OUT_INDEX:
        k = tl.load(out_idx_ptr + b.to(tl.int64) * out_idx_stride_b, mask=mask, other=0).to(tl.int32)
    else:
    ...
    rot_base = rot_ptr + b.to(tl.int64) * rot_stride_b
    ...
    pose_weight = tl.load(pose_weight_ptr + b.to(tl.int64), mask=mask, other=0.0).to(tl.float32)
    w = pose_weight * pixel_weight

    input_base = (
        input_cplx_ptr
        + b.to(tl.int64) * input_stride_b
        + pix_lin.to(tl.int64) * input_stride_p
    )
    input_re = tl.load(input_base + 0 * input_stride_cplx, mask=mask, other=0.0).to(tl.float32)
    input_im = tl.load(input_base + 1 * input_stride_cplx, mask=mask, other=0.0).to(tl.float32)

    if HAS_MODULATION:
        modulation_base = (
            modulation_ptr
            + b.to(tl.int64) * modulation_stride_b
    ...
    v_lin = tl.where(m000, v_lin_raw, 0)
    ...
    v_lin = tl.where(m001, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
    ...
    v_lin = tl.where(m010, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
    ...
    v_lin = tl.where(m011, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
    ...
    v_lin = tl.where(m100, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
    ...
    v_lin = tl.where(m101, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
    ...
    v_lin = tl.where(m110, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
    ...
    v_lin = tl.where(m111, v_lin_raw, 0)
    v_base = out_numer_base + v_lin * numer_stride_v
