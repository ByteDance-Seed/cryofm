...
@triton.jit
def central_slice_sample_ndhwc_ch2_fwd_kernel(
    input_ptr,
    rotation_ptr,
    output_ptr,
    N_POSE,
...
@triton.jit
def central_slice_sample_ndhwc_ch2_bwd_input_kernel(
    grad_output_ptr,
    rotation_ptr,
    grad_input_ptr,
    N_POSE,
    ...
    grad_re = tl.load(grad_output_ptr + go_base + 0 * grad_output_stride_c, mask=mask, other=0.0).to(tl.float32)
    grad_im = tl.load(grad_output_ptr + go_base + 1 * grad_output_stride_c, mask=mask, other=0.0).to(tl.float32)
    ...
    tl.atomic_add(grad_input_ptr + base000 + 0 * grad_input_stride_c, w000 * grad_re, mask=m000)
    tl.atomic_add(grad_input_ptr + base000 + 1 * grad_input_stride_c, w000 * grad_im, mask=m000)
    tl.atomic_add(grad_input_ptr + base001 + 0 * grad_input_stride_c, w001 * grad_re, mask=m001)
    tl.atomic_add(grad_input_ptr + base001 + 1 * grad_input_stride_c, w001 * grad_im, mask=m001)
    tl.atomic_add(grad_input_ptr + base010 + 0 * grad_input_stride_c, w010 * grad_re, mask=m010)
    tl.atomic_add(grad_input_ptr + base010 + 1 * grad_input_stride_c, w010 * grad_im, mask=m010)
    tl.atomic_add(grad_input_ptr + base011 + 0 * grad_input_stride_c, w011 * grad_re, mask=m011)
    tl.atomic_add(grad_input_ptr + base011 + 1 * grad_input_stride_c, w011 * grad_im, mask=m011)
    tl.atomic_add(grad_input_ptr + base100 + 0 * grad_input_stride_c, w100 * grad_re, mask=m100)
    tl.atomic_add(grad_input_ptr + base100 + 1 * grad_input_stride_c, w100 * grad_im, mask=m100)
    tl.atomic_add(grad_input_ptr + base101 + 0 * grad_input_stride_c, w101 * grad_re, mask=m101)
    tl.atomic_add(grad_input_ptr + base101 + 1 * grad_input_stride_c, w101 * grad_im, mask=m101)
    tl.atomic_add(grad_input_ptr + base110 + 0 * grad_input_stride_c, w110 * grad_re, mask=m110)
    tl.atomic_add(grad_input_ptr + base110 + 1 * grad_input_stride_c, w110 * grad_im, mask=m110)
    tl.atomic_add(grad_input_ptr + base111 + 0 * grad_input_stride_c, w111 * grad_re, mask=m111)
    tl.atomic_add(grad_input_ptr + base111 + 1 * grad_input_stride_c, w111 * grad_im, mask=m111)