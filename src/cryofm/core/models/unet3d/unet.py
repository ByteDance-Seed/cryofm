...
class UNet3DModel(ModelMixin, ConfigMixin):
    ...
    Parameters:
        ...
        mid_block_type (`str`, *optional*, defaults to `"UNetMidBlock3D"`):
            Block type for middle of UNet, it can be either `UNetMidBlock3D` or `UnCLIPUNetMidBlock3D`.
        ...
        resnet_time_scale_shift (`str`, *optional*, defaults to `"default"`): Time scale shift config
            for ResNet blocks (see [`ResnetBlock3D`]). Choose from `default` or `scale_shift`.
    ...
    def forward(
        ...
        Args:
            ...
            down_block_additional_residuals (`tuple[torch.Tensor]`, *optional*, defaults to `None`):
                Additional residuals from ControlNet for down blocks.
            mid_block_additional_residual (`torch.Tensor`, *optional*, defaults to `None`):
                Additional residual from ControlNet for mid block.
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`UNet3DOutput`] instead of a plain tuple.

        Returns:
            [`UNet3DOutput`] or `tuple`:
                If `return_dict` is True, an [`UNet3DOutput`] is returned, otherwise a `tuple` is
        ...
        # 5. ControlNet residuals
        if down_block_additional_residuals is not None:
            down_block_res_samples = list(down_block_res_samples)
            for i, residual in enumerate(down_block_additional_residuals):
                down_block_res_samples[i] = down_block_res_samples[i] + residual
