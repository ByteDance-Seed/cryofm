...
1. Data variable naming:
   - raw_* : raw data (input state)
   - resized_* : data resized to the model pixel size
   - cropped_* : cropped data
   - patched_* : data after patching/splitting into chunks
   - denoised_* : denoised data
   - final_* : final output data

2. Processing parameter naming:
   - fsc_* : FSC-related parameters
   - mask_* : mask-related parameters
   - wavelet_* : wavelet-related parameters
   - energy_* : energy-related parameters

3. Function naming:
   - compute_* : calculation functions
   - build_* : preparation functions
   - apply_* : application functions
...
from cryofm.projects.cryofm2.sampling_helper import (
    change_dir, auto_find_base_dir, InputData, timer, compute_fsc_values, build_fourier_mask,
    compute_wavelet_weights, apply_bbox_crop, apply_normalize_and_patchify, build_target_energy_and_fsc_volume,
    apply_posterior_sampling, apply_spisonet_mixing, apply_post_processing, apply_fsc_weighting, MODEL_VOXEL_SIZE,
...
def load_model(args, device):
    ...
    lit_model = CryoFM2Uncond.load_from_safetensors(osp.join(args.model_dir, "model.safetensors"), cfg=cfg).to(device)
    unet = lit_model.model.to(device).eval()    
    scheduler = lit_model.noise_scheduler
    return lit_model, unet, scheduler
...
def prepare_input(args):
    ...
    if input_path1_file_type != input_path2_file_type:
        raise ValueError(f"Half map file type must be the same, got {input_path1_file_type} and {input_path2_file_type}")
    ...
    if args.input_path is not None:
        ...
        if input_path_file_type == "MRC":
            vol, _, _ = load_mrc(args.input_path)
...
def _single_map_process(
    ...
    bbox_output = apply_posterior_sampling(
        args=args, unet=unet, scheduler=scheduler,
